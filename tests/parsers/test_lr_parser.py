...
class TestLRParser(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(parser.conflicts, [])

    def test_set_action_tracks_conflicts(self):
        """Test that assigning conflicting ACTION entries records conflicts."""
        ...
        parser._set_action(0, 'id', ('shift', 1))
        parser._set_action(0, 'id', ('shift', 1))
        parser._set_action(0, 'id', ('reduce', 'E', ['id']))

        self.assertEqual(parser.action[(0, 'id')], ('shift', 1))
        self.assertEqual(len(parser.conflicts), 1)
        self.assertEqual(parser.conflicts[0]['state'], 0)
        self.assertEqual(parser.conflicts[0]['symbol'], 'id')
        self.assertEqual(parser.conflicts[0]['conflict_type'], 'shift-reduce')
